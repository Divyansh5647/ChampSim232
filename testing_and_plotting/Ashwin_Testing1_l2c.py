...
def func(type, size):
    ...
    if(type==1): # log
        ...
        return 20*int(math.log2(size/(2048*16) + 1))
    elif(type==2): # linear
...
for llc_size in [2048*16]:
    for llc_ratio in [128]:
        for l2c_size in [1024*8]:
            for l2c_way in [4,8,16]:
                l2c_set=l2c_size//l2c_way
                for l1d_set in [64]:
                    for l1d_way in [12]:
                        ...
                        with open("inc/cache.h", 'w') as f:
                            ...
                            data2 = f'{data[:llc_start]}#define LLC_SET NUM_CPUS*{int(math.sqrt(llc_size*llc_ratio))}\n#define LLC_WAY {int(math.sqrt(llc_size/llc_ratio))}\n{data[llc_end:]}'
                        ...
                        with open(f"Ashwin_Results_1_l2c_{tracename}_{repl}_{size1}_{size2}.txt", 'a') as f:
                            f.write(f'{int(math.sqrt(llc_size*llc_ratio))}, {int(math.sqrt(llc_size/llc_ratio))}, {l2c_set}, {l2c_way}, {l1d_set}, {l1d_way}, {ipc_val},\n {llc_miss_freq}, {llc_latency}, {l2c_miss_freq}, {l2c_latency}, {l1d_miss_freq}, {l1d_latency}\n\n')
