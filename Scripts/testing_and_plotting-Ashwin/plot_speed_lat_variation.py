...
for_legend = ["log","slope 1/512*4","slope 1/512*8","slope 1/512*16","slope 1/512*32","slope 1/512*64"]

name = "kissat-inc-high-30K-1802B.champsimtrace.xz"
arr_ipc_1 = [0.318813,0.307637,0.29801,0.287873]
arr_ipc_2 = [0.317369,0.307637,0.291876,0.264798]
arr_ipc_3 = [0.311706,0.307637,0.30181,0.290167]
arr_ipc_4 = [0.308948,0.307637,0.307021,0.304734]
arr_ipc_5 = [0.307587,0.307637,0.309691,0.312566]
arr_ipc_6 = [0.307587,0.307637,0.311042,0.316617]
...
x_axis_labels_4 = ['(2048,16)','(4096,16)','(8192,16)']
x_axis_labels_5 = ['(2048,16)','(4096,16)','(8192,16)']




plt.title(f'IPC with cache size')
my_min = np.min(arr_ipc_1+arr_ipc_2+arr_ipc_3+arr_ipc_4+arr_ipc_5+arr_ipc_6)
my_max = np.max(arr_ipc_1+arr_ipc_2+arr_ipc_3+arr_ipc_4+arr_ipc_5+arr_ipc_6)
diff = my_max-my_min
plt.plot(x_axis_vals,arr_ipc_1,"-o")
plt.plot(x_axis_vals,arr_ipc_2,"-o")
plt.plot(x_axis_vals,arr_ipc_3,"-o")
plt.plot(x_axis_vals,arr_ipc_4,"-o")
plt.plot(x_axis_vals,arr_ipc_5,"-o")
plt.plot(x_axis_vals,arr_ipc_6,"-o")
...
plt.xticks(x_axis_vals,[(1024,16),(2048,16),(4096,16),(8192,16)])
