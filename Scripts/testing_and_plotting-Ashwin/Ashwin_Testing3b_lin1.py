...
for llc_set in [1024,2048,4096,8192]:
    for llc_way in [16]:
        for l2c_set in [2048]:
            for l2c_way in [8]:
                for l1d_set in [128]:
                    for l1d_way in [12]:
                        ...
                        with open("inc/cache.h", 'w') as f:
                            ...
                            data5 = f'{data4[:llc_latency_s]}#define LLC_LATENCY {func(2,llc_set*llc_way)}{data4[llc_latency_e:]}'
                        ...
                        with open(f"Ashwin_Results_3b_lin_{tracename}_{repl}_{size1}_{size2}.txt", 'a') as f:
